...
useEffect(() => {
  ...
  const addAvailabilitySlot = () => {
    setAvailability([
      ...availability,
      { day: "Monday", from: "09:00", to: "10:00" },
    ]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/tutor/apply`,
        formData,
        {
          headers: {
          ...
          <Card className="w-full max-w-md border-none shadow-2xl">
            <CardHeader className="text-center pb-8">
              ...
              <CardDescription className="text-base mt-3">
                Thank you for applying to become a tutor at Level Up. We'll
        ...
        >
          <Card className="border-none shadow-2xl backdrop-blur-sm bg-white/95">
            <CardHeader className="text-center pb-8">
              ...
              <CardDescription className="text-base mt-3">
                Join our community of educators and help students achieve their
                academic goals
              </CardDescription>
                    ...
                    >
                      <div className="space-y-3">
                        {TEACHING_STYLES.map((style) => (
                          <div
                            key={style.value}
                            className="flex items-start space-x-3 p-4 rounded-lg border-2 hover:border-purple-300 transition-colors cursor-pointer"
                          >
                            <RadioGroupItem
                              value={style.value}
                              id={style.value}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {availability.map((slot, index) => (
                      <div
                        key={index}
                        className="flex gap-2 items-end p-3 rounded-lg border-2 bg-gray-50"
                      >
